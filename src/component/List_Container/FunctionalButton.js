 ...
 * @Last Modified time: 2018-07-28 09:49:25
 */
    ...
    render = () => {
    	...
    	for(let item of power) {
    		if(item.includes('Add')) {
    			extraAdd = (
    				<List.Item>
    					<Button type='primary' onClick={ this.handleOnAdd }>新增</Button>
    				</List.Item>
    			);
    		}
    	}
    	...
    	return (
    		<div className='FunctionalButton'>
    			<div className='sc-extend-add' onClick={ () => this.setState({ modalVisible: !modalVisible }) } style={{ display: visible ? '' : 'none' }}>
    				<i className='sc-extend-add-icon'>+</i>
    			</div>

    			<Modal popup animationType='slide-up' visible={ modalVisible } onClose={ () => { this.setState({ modalVisible: !modalVisible }); } }>
    				<List renderHeader={ () => <div>按字段排序</div> }>
    					{
    						datas.map((item, index) => (
    							<List.Item extra={ item.status } key={ index } arrow={ item.direction } onClick={ () => this.handleOnClick(item) }>{ item.text }</List.Item>
    						))
    					}
    					{ extraAdd }
    				</List>
