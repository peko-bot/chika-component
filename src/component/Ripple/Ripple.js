...
export default class Ripple extends React.Component {
    ...
    componentDidMount = () => {
        // 动画结束后移除样式 mark
        this.ripple.removeEventListener('transitionend', this.handle_transitionend, false);
        this.ripple.addEventListener('transitionend', this.handle_transitionend, false);
    }

    init = element => {
        if(!element) return;

        element.removeEventListener('click', this.handle_click, false);
        element.addEventListener('click', this.handle_click, false);
    }

    handle_transitionend = e => {
