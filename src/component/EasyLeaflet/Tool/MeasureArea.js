...
export class MeasureArea extends MultilateralDraw {
	constructor (props) {
		super(props);
		this.initialize();
	}
    ...
    click = (e) => {
    	if (!this.options.draw) {
    		this.firstClick(e, 'polygon', { draw: defaultAreaStyle, moveDraw: moveAreaStyle, pointStyle: measureAreaPoint });
    		...
    		if (time - this.options.time < 330) {
    			...
    			setTimeout(() => {
    				for (let p in area) { area[p] = null; delete area[p]; }
    ...
    move = (e) => {
    	!!this.options.moveDraw && !!this.options.prePoint && !!this.options.thisPoint && this.options.moveDraw.setLatLngs([this.options.prePoint.latlng, this.options.thisPoint.latlng, e.latlng]);
    }
}