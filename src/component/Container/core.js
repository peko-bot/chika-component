...
class ContainerCore extends React.Component {
  ...
  getConfig = () => {
    ...
    ajax({
      ...
      success: ({ data }) => {
        // 无论如何都会有查看的权限
        let { power, tablefieldconfig } = data;
  ...
  search = searchType => {
    ...
    data = url
      ? Object.assign(
          ...
          {
            RequestUrl,
            RequestParams: Object.assign({}, data, RequestParams),
            RequestMethod,
          },
        )
   ...
   * left: order * -100%
   */
  handleDetailDatas = (datas, value, mainKey) => {
    if (datas.length == 0) {
      return datas;
    }
  ...
  handleItemEdit = (mainValue, type) => {
    ...
    this.listDatas = this.handleDetailDatas(
      this.listDatas,
      mainValue,
      this.mainKey,
    );
    ...
    switch (controltype) {
      case 1: // 文本框
        option = {
          ...
          /* 设置默认值 */
          initialValue:
            this.handleInputValue(editParam[fname], item) ||
            this.handleInputValue(defaultvalue, item),
          rules: [
        ...
        element =
          type == 'search' ? (
            <InputItem
              {...params}
              onChange={e => this.handleInput(e, 'searchParam', fname)}
              value={searchParam[fname]}
            >
              {fvalue}
            </InputItem>
          ) : (
      ...
      case 2: // 时间控件
        ...
        option = {
          onChange: date =>
            this.handleDate(date, 'editParam', fname, dateformat),
        ...
        element =
          type == 'search' ? (
            <DatePicker
              key={`case_2_datePicker_${index}`}
              value={searchParam[fname] ? new Date(searchParam[fname]) : null}
              onChange={date =>
      ...
      case 3: // 下拉框
        ...
        element =
          type == 'search' ? (
            <Picker
              {...params}
              onChange={value => this.handleSelect(value, 'searchParam', fname)}
              value={[searchParam[fname]]}
            >
              <List.Item arrow="horizontal">{fvalue}</List.Item>
            </Picker>
      ...
      case 5: // 多选框
        ...
        element = (
          <Accordion key={`case_5_accordion_${index}`}>
            <Accordion.Panel
              header={fvalue}
              key={`case_5_accordion_panel_${index}`}
            >
      ...
      case 14: // 地图选点
        ...
        if (type == 'detail') {
          element = (
            <List.Item
              key={`case_14_listItem_detail_${index}`}
              extra={detailItem[fname] ? detailItem[fname].split('|')[2] : ''}
            >
              {fvalue}
            </List.Item>
          );
          ...
          this.state.editParam = Object.assign({}, editParam, {
            [newLng]: lng,
            [newLat]: lat,
            [newAddress]: address,
          });
          ...
          element.push(
            <List.Item
              key={`case_14_listItem_lng_${index}`}
          ...
          element.push(
            <List.Item
              key={`case_14_listItem_lat_${index}`}
          ...
          element.push(
            <List.Item
              key={`case_14_listItem_address_${index}`}
              arrow="horizontal"
              onClick={() => getLatng({ lat, lng })}
              extra={address}
            >
              地址
            </List.Item>,
      ...
      case 99: // label，基本就是给详情页用的
        element = (
          <List.Item
            key={`case_99_listItem_${index}`}
            extra={this.handleInputValue(detailItem[fname], item)}
          >
            {fvalue}
          </List.Item>
        );
  ...
  handlePullLoad = () => {
    if (this.listDatas.length < this.recordCount) {
      ...
      this.setState(
        { pullLoad: true, searchParam: Object.assign(searchParam, param) },
        () => {
  ...
  handleOnMapClose = latng => {
    ...
    editParam = Object.assign({}, editParam, {
      [this.mapFname]: `${lng}|${lat}|${address}`,
    });
  ...
  render = () => {
    ...
    let sidebar = (
      <List>
        <List.Item>
          <Button onClick={this.handleSearch} loading={searchLoading}>
            确定
          </Button>
        </List.Item>
        {config.map((item, i) =>
